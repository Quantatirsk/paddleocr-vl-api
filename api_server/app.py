...
async def create_zip_response(output_dir: str, output_files: Dict[str, Dict[str, str]]) -> StreamingResponse:
    ...
    扁平结构：所有文件直接在 ZIP 根目录
    """
    zip_buffer = io.BytesIO()

    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        # 添加输出文件（JSON, Markdown 等）- 直接放在根目录
        for _, files in output_files.items():
            for _, file_path in files.items():
                if os.path.exists(file_path):
                    ...
                    logger.debug(f"Added to ZIP (root): {arcname}")
        ...
        if imgs_dir.exists() and imgs_dir.is_dir():
            ...
            for img_file in imgs_dir.rglob("*"):
                if img_file.is_file():
                    # 保持相对于 output_dir 的目录结构（保留 imgs/ 前缀）
                    arcname = str(img_file.relative_to(Path(output_dir)))
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=ocr_results.zip"}
    )
